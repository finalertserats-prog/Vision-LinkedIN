...
def _build_diagram_prompt(post_text: str) -> str:
    ...
    return (
        ...
        "Read the post below. DEFAULT TO 'NONE'. A diagram is the rare exception, "
        "not the habit: most posts are better served by art, and a needless "
        "flowchart makes the writing look mechanical and stale. Answer with a "
        "diagram ONLY if the post's argument genuinely RESTS on a concrete "
        "PROCESS, DECISION FLOW, ARCHITECTURE, or set of STEPS or ROUTES, AND the "
        "picture carries information the sentences cannot. Ask yourself: would a "
        "reader who saw only this diagram learn something the post does not "
        "already say plainly? If the answer is anything short of a clear yes - or "
        "if the post is a reflection, an opinion, a story, or a single idea - "
